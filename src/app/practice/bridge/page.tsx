...
export default function BridgePracticePage() {
  ...
  // Generate options when the test starts or the index changes
  const generateOptions = useCallback((index: number, batch: BridgeWord[]) => {
    if (!batch[index]) return;
    const correctWord = batch[index];
    const otherWords = BRIDGE_WORDS.filter((w) => w.id !== correctWord.id);

    const shuffledOthers = [...otherWords].sort(() => Math.random() - 0.5);
    const selectedOthers = shuffledOthers.slice(0, 3);

    const finalOptions = [...selectedOthers, correctWord].sort(
      () => Math.random() - 0.5,
    );
    setCurrentOptions(finalOptions);
  }, []);

  // Sync options with current index
  useEffect(() => {
    if (isTesting && testBatch.length > 0) {
      generateOptions(currentTestIndex, testBatch);
    }
  }, [isTesting, currentTestIndex, testBatch, generateOptions]);
  ...
  const startTest = () => {
    ...
    setIsTesting(true);
    setShowResult(false);
    // Initial options will be generated by the useEffect
  };
        ...
        <div className='flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12'>
          ...
          <div className='flex items-center gap-4 bg-secondary p-2 rounded-[24px]'>
            <div className='flex flex-col items-center px-6 py-2'>
              <span className='text-[10px] font-black text-indigo/30 uppercase tracking-[0.2em]'>
                Current Level
              </span>
              <span className='text-2xl font-black text-indigo'>
                {bridgeLevel}
              </span>
            </div>
            <button
        ...
        <div className='flex gap-4 overflow-x-auto pb-6 -mx-4 px-4 no-scrollbar'>
          {BRIDGE_CATEGORIES.map((cat) => (
            <button
              ...
              className={`flex items-center gap-3 px-6 py-4 rounded-[20px] whitespace-nowrap transition-all ${
              ...
              >
                <div className='flex items-start justify-between mb-4'>
                  <div className='space-y-1'>
                    ...
                    <h3 className='text-2xl font-bold text-indigo'>
                      {word.hindi}
                    </h3>
                  </div>
                ...
                <div className='flex items-center gap-4 py-4 mb-4 border-y border-gold-dark/5'>
                  ...
                  <div className='space-y-1'>
                    <p className='text-kannada text-2xl font-bold text-terracotta'>
                      {word.kannada}
                    </p>
                    <p className='text-xs font-medium text-indigo-light italic'>
                ...
                <div className='flex items-center justify-between text-sm'>
                  <span className='font-bold text-indigo/60'>
                    {word.meaning}
                  </span>
                  {word.note && (
                    <span className='text-[10px] text-green font-bold uppercase'>
                      {word.note}
                    </span>
                  )}
                </div>
